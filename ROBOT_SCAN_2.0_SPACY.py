...
def render_full_equip_ui():
    ...
    with col1:
        fe_input_folder = st.text_input(
            ...
            value=r"C:\Users\palacif\OneDrive - Air Products and Chemicals, Inc\Desktop\PROVA\SCAN_IA_PROJECT\assistent_digital\imatges_prova\INPUT_ALBARANS_OPENTEXT",
            key="fe_input_folder"
        )
    with col2:
        fe_hot_folder = st.text_input(
            "Carpeta Hot Folder (Enterprise Scan)",
            value=r"C:\EnterpriseScan\IN\Albarans",
            key="fe_hot_folder"
        )
        ...
        with col1:
            with st.container(border=True):
                ...
                if st.button("🎤 Veu", key=f"voice_btn_{manual_info['original_name']}", help="Captura entrada de veu"):
                    ...
                    if voice_text:
                        st.session_state[manual_key] = voice_text.upper()
                        st.rerun()
    ...
    if st.session_state.get('pro_scan_confirmation_needed'):
        ...
        INPUT_FOLDER = st.session_state.get("fe_input_folder") or r"C:\Users\palacif\OneDrive - Air Products and Chemicals, Inc\Desktop\PROVA\SCAN_IA_PROJECT\assistent_digital\imatges_prova\INPUT_ALBARANS_OPENTEXT"
        HOT_FOLDER = st.session_state.get("fe_hot_folder") or r"C:\EnterpriseScan\IN\Albarans"
        try:
...
if st.session_state.get('pro_scan_confirmation_needed'):
    ...
    if files_omitted:
        col_met2.metric("Fitxers omesos", len(files_omitted))
    INPUT_FOLDER = r"C:\Users\palacif\OneDrive - Air Products and Chemicals, Inc\Desktop\PROVA\SCAN_IA_PROJECT\assistent_digital\imatges_prova\INPUT_ALBARANS_OPENTEXT"
    HOT_FOLDER = r"C:\EnterpriseScan\IN\Albarans"
    try:
        os.makedirs(HOT_FOLDER, exist_ok=True)
        with st.spinner(f"Enviant fitxers a '{HOT_FOLDER}'..."):
            sent_count = 0
