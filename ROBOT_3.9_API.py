...
def _llm_gemini(text_brut_ocr: str) -> str | None:
    ...
    try:
        ...
        prompt = f"{PROMPT_SISTEMA_ENTREGA}\n\nText OCR:\n---\n{text_brut_ocr}"
...
def get_dn_from_llm(text_brut_ocr: str) -> str | None:
    """
...
def llm_fallback_update(summary: dict, full_text: str) -> dict:
    """
    Si no hi ha codi detectat, prova LLM sobre full_text.
    Si troba un VBELN vàlid, marca 'detected' i 'detected_code' al summary.
    """
    try:
        if not summary.get("detected") and (full_text or "").strip():
            dn = get_dn_from_llm(full_text)
            if dn:
...
def process_document(path: str, tipus_sel: str = 'a', timeout: float = 60.0, mode: str = "FAST") -> Tuple[Dict[str, Any], str, List[str]]:
    ...
    try:
        ...
        try:
            ...
            payload = q.get()
            # >>> LLM fallback després de l'anàlisi principal
            s = payload.get('summary', {"detected": False})
            full_text = payload.get('full_text', '')
            s = llm_fallback_update(s, full_text)
            payload['summary'] = s
