...
ROBOT_SCAN_4.2.1
================
- Correccions **CRÍTIQUES** de REGEX (ES|PT, ALNUM10, inline ES/PT i ship-to).
- Verificació d'identació (4 espais, sense tabs).
- Manté 4.2: extracció per etiqueta "Número de entrega : <valor>", validació ampliada,
  FAST amb DataMatrix i rotacions completes, eines de barra lateral (Organitzar / Reconeixer).

Execució recomanada:
    streamlit run ROBOT_4.2.1.py
"""
...
log = logging.getLogger("robot_scan_4_2_1")

# ---------- Config Streamlit ----------
st.set_page_config(
    page_title="ROBOT SCAN 4.2.1",
    layout="wide",
...
def _valid_code(candidate: str) -> Optional[str]:
    ...
    if RE_ES_PT_VALIDATOR.fullmatch(c):
        return c
    if RE_ALBARA_ALNUM10.fullmatch(c):
        return c
    return None
...
def _decode_barcodes_with_upsample(img: Image.Image, rotations: List[int], upsample_x: float = 1.0,
                                   use_otsu: bool = True, use_dmtx: bool = True) -> List[Dict[str, str]]:
...
def _extract_delivery_from_label(text: str) -> str:
    """Extreu el valor al costat de l'etiqueta 'Número de entrega :' (tolerant a espais i :)"""
    if not text:
        return ""
    t = text.upper()
    # Accepta A-Z0-9 i -_/ fins a 20 caràcters
...
def analyze_pdf(pdf_path: str, dpi: int = 300, ocr_lang: str = 'spa+eng', max_pages: Optional[int] = None, mode: str = "FAST") -> FileResult:
    ...
    for i, page in enumerate(doc):
        ...
        # 5) OCR com a darrer recurs (si poc text natiu)
...
def process_document(path: str, tipus_sel: str = 'a', timeout: float = 60.0, mode: str = "FAST") -> Tuple[Dict[str, Any], str, List[str]]:
    ...
    try:
        # Si és imatge, convertir temporalment a PDF
        ...
        # Multiprocessing (amb fallback a threading)
        try:
...
def enviar_a_enterprise_scan(file_bytes: bytes, original_name: str, delivery_number: str, outbox: str) -> str:
    if not (RE_ALBARAN_VALIDATOR.fullmatch(delivery_number)
            or RE_ES_PT_VALIDATOR.fullmatch(delivery_number)
            or RE_ALBARA_ALNUM10.fullmatch(delivery_number)):
...
# ---------- EINES: Organitzar / Reconeixer ----------
def organitzar_pdfs(directori_principal: str) -> None:
    """Mou 'Page00000000.pdf' fora de subcarpetes i neteja."""
...
def reconeixer_pdfs(directori: str, mode: str = 'FAST', rename: bool = False) -> pd.DataFrame:
    """Analitza tots els PDFs/imatges d'una carpeta i retorna un DataFrame amb el codi detectat.
    Si rename=True, copia/renombra cada arxiu amb _safe_name(codi, ext).
    """
    root = Path(directori)
    if not root.is_dir():
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        with st.expander("⚙️ Diagnòstic", expanded=False):
            ...
            st.info("Executa amb `streamlit run` (no amb `python`).")
