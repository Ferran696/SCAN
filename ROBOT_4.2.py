...
ROBOT_SCAN_4.2
==============
- Correcció d'indentació i neteja de codi (evita errors del tipus IndentationError).
- Validador ampliat: admet codis d'entrega alfanumèrics de 10 caràcters (p. ex. EPYYFFFF5M).
- Extracció prioritzada per etiqueta: "Número de entrega : <valor>" (text i OCR).
- Regex ship-to més tolerant: permet múltiples ":" i espais.
- Mode FAST millorat: DataMatrix activat i rotacions 0/90/180/270 (ROI i full-page).
- Eines a la barra lateral: 🛠️ Organitzar PDFs i 🧪 Reconeixer PDFs (lot + CSV + renombrat opcional).

**Com executar:**
    streamlit run ROBOT_4.2.py

Nota: El missatge "missing ScriptRunContext" apareix si s'executa el fitxer
amb `python ROBOT_4.2.py`. Usa `streamlit run` per evitar-lo.
"""
...
log = logging.getLogger("robot_scan_4_2")

# --- Configuració de pàgina Streamlit ---
st.set_page_config(
    page_title="ROBOT SCAN 4.2",
    layout="wide",
...
RE_ALBARA_ALNUM10 = re.compile(r"^[A-Z0-9]{10}$")
RE_ESPT_INLINE = re.compile(r"\b(?:ES|PT)[A-Z0-9]{7}\b", re.IGNORECASE)
RE_SHIP_TO = re.compile(
    r"(?i)(?:ship\s*to|destinatari[oa]|destinatario\s*de\s*mercanc[ií]a)\s*[:\-\s]*([0-9]{5,9})"
)
...
MODE_PRESETS = {
    ...
    "FAST": {
        ...
        "roi_dpi": 520,
        "roi_upsample_x": 2.0,
        "rot_roi": [0, 90, 180, 270],   # 4.2
        "rot_full": [0, 90, 180, 270],  # 4.2
...
def _valid_code(candidate: str) -> Optional[str]:
    ...
    if RE_ES_PT_VALIDATOR.fullmatch(c):
        return c
    if RE_ALBARA_ALNUM10.fullmatch(c):  # 4.2
        return c
    return None
...
def _extract_text_hits(text: str) -> Dict[str, List[str]]:
    ...
    hits["delivery_10d"] = [
        c
        for c in (_valid_code(x) or x for x in hits["delivery_10d"])
        if re.fullmatch(r"8\d{9}", c or "")
    ]
    return hits
...
# --- Extracció per etiqueta ---
def _extract_delivery_from_label(text: str) -> str:
    """Extreu el valor al costat de l'etiqueta 'Número de entrega :'
    Retorna string sanititzat o buit si no troba res.
    """
    if not text:
        return ""
    t = text.upper()
    m = re.search(r"N[ÚU]MERO\s+DE\s+ENTREGA\s*:\s*([A-Z0-9\-_/]{8,20})", t)
    if m:
        return _sanitize_barcode_text(m.group(1))
    return ""
...
def analyze_pdf(pdf_path: str, dpi: int = 300, ocr_lang: str = 'spa+eng', max_pages: Optional[int] = None, mode: str = "FAST") -> FileResult:
    ...
    def log_event(evts: List[TraceEvent], step: str, detail: str = "") -> None:
    ...
    for i, page in enumerate(doc):
        ...
        # 1) PRIORITZA etiqueta "Número de entrega:"
        label_code = _extract_delivery_from_label(text_layer)
        if label_code:
            dc = _valid_code(label_code) or label_code
            detected_code = dc
            pages.append(PageResult(i, strategy, char_count, img_count, text_layer, barcodes, hits))
            log_event(trace, "hit_label", f"page={i} code={dc}")
            break

        # 2) Patrons clàssics (8 dígits inicials amb 8, ES/PT + 7)
        ...
        # 3) ROI (al voltant de paraules clau)
        if _time_left():
            strategy = 'barcode-roi'
            rects: List['fitz.Rect'] = []
            for kw in ROI_LABELS:
                try:
                    rects.extend(page.search_for(kw, flags=1))  # case-insensitive
                except Exception:
                    pass
        ...
        if preset["try_ocr"] and char_count < 30 and _time_left():
            try:
                ...
                # Prioritza etiqueta en OCR
                label_code = _extract_delivery_from_label(ocr_txt)
                if label_code:
                    dc = _valid_code(label_code) or label_code
                    detected_code = dc
                    pages.append(PageResult(i, strategy, char_count, img_count, ocr_txt, barcodes, hits_ocr))
                    log_event(trace, "hit_label_ocr", f"page={i} code={dc}")
                    break
...
def _analysis_subprocess(tmp_pdf_path: str, mode: str, q_out) -> None:
    try:
...
def process_document(path: str, tipus_sel: str = 'a', timeout: float = 60.0, mode: str = "FAST") -> Tuple[Dict[str, Any], str, List[str]]:
    ...
    try:
        # Convertir imatge a PDF temporal
...
# --- Suport UI+Files ---
def _create_placeholder_image(width: int = 400, height: int = 300, text: str = "Sense previsualització") -> Image.Image:
...
def enviar_a_enterprise_scan(file_bytes: bytes, original_name: str, delivery_number: str, outbox: str) -> str:
    if not (RE_ALBARAN_VALIDATOR.fullmatch(delivery_number) or RE_ES_PT_VALIDATOR.fullmatch(delivery_number) or RE_ALBARA_ALNUM10.fullmatch(delivery_number)):
...
# --- Visualitzador interactiu ---
from streamlit.components.v1 import html as st_html

def show_interactive_viewer(pil_image: Image.Image, height: int = 650) -> None:
    buf = BytesIO()
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        with st.expander("⚙️ Diagnòstic", expanded=False):
            ...
            st.info("La captura de veu utilitza serveis de tercers (Google). Executa amb `streamlit run`.")

        # --- Eina: Organitzar PDFs ---
        with st.expander("🛠️ Eina: Organitzar PDFs"):
            st.info("Organitza els PDFs de subcarpetes (Page00000000.pdf) movent-los i netejant carpetes.")
            dir_organitzar = st.text_input("Ruta del directori (ex: .../IMPORT_ESCAN)", key="tool_organitzar_dir")
            if st.button("Executar Organització", key="tool_organitzar_btn"):
                if dir_organitzar:
        ...
        # --- Eina: Reconeixer PDFs ---
        with st.expander("🧪 Eina: Reconeixer PDFs"):
            st.info("Analitza en lot una carpeta, exporta CSV i pot copiar/renombrar per codi detectat.")
            dir_rec = st.text_input("Ruta del directori a reconèixer", key="tool_recon_dir")
            mode_rec = st.selectbox("Mode per al reconeixement", ["ULTRAFAST", "FAST", "ACCURATE"], index=1, key="tool_recon_mode")
            do_rename = st.checkbox("Copiar i renombrar amb el codi detectat", value=False, key="tool_recon_rename")
            if st.button("Reconèixer", key="tool_recon_btn"):
                if dir_rec:
                    with st.spinner("Processant carpeta..."):
                        df = reconeixer_pdfs(dir_rec, mode=mode_rec, rename=do_rename)
                        if not df.empty:
                            st.dataframe(df, use_container_width=True)
                            csv_bytes = df.to_csv(index=False).encode('utf-8')
                            st.download_button("⬇️ Descarrega CSV de reconeixement", data=csv_bytes, file_name="reconeixement.csv", mime="text/csv")
                else:
                    st.warning("Si us plau, especifica un directori.")
...
def render_progress_stats() -> None:
...
# --- UI: Entrada manual ---
def apply_pending_voice_input() -> None:
...
def handle_manual_assign(code: str) -> None:
...
def render_manual_input_ui() -> None:
    ...
    manual_code = st.text_input("Introdueix el codi (8... / ES/PT... / 10 alfanumèrics):", key="manual_code_input").upper()
    is_valid = bool(
        RE_ALBARAN_VALIDATOR.fullmatch(manual_code)
        or RE_ES_PT_VALIDATOR.fullmatch(manual_code)
        or RE_ALBARA_ALNUM10.fullmatch(manual_code)
    )
...
def handle_confirmation_continue() -> None:
    st.session_state.app_status = "SENDING"


def handle_confirmation_stop() -> None:
    reset_app_state()


def render_confirmation_ui() -> None:
...
def render_sending_ui() -> None:
    ...
    for i, result in enumerate(files_to_send):
        ...
        try:
            ...
            st.write(f"✅ Enviat: `{name}` -> `{os.path.basename(dest)}`")
            sent += 1
...
def pro_scan_workflow() -> None:
    ...
    try:
        ...
        code = summary.get("detected_code")
        info = {
...
# --- Eines: Organitzar i Reconeixer ---
def organitzar_pdfs(directori_principal: str) -> None:
    ...
    try:
        if not any(ruta_base.iterdir()):
            ...
            if not any(f.is_dir() for f in ruta_base.iterdir()):
                shutil.rmtree(ruta_base)
...
def reconeixer_pdfs(directori: str, mode: str = 'FAST', rename: bool = False) -> pd.DataFrame:
    root = Path(directori)
    if not root.is_dir():
        st.error(f"La ruta '{directori}' no existeix o no és carpeta.")
        return pd.DataFrame()
    files = sorted([p for p in root.iterdir() if p.suffix.lower() in ALLOWED_EXTS])
    rows: List[Dict[str, Any]] = []
    prog = st.progress(0.0, text=f"Analitzant {len(files)} fitxers...")
    for i, p in enumerate(files):
        try:
            res = analyze_pdf(str(p), mode=mode)
            code = res.summary.get('detected_code')
            det = bool(res.summary.get('detected'))
            if rename and code:
                dest = p.with_name(_safe_name(code, p.suffix))
                try:
                    shutil.copy2(str(p), str(dest))
                except Exception as e:
                    st.warning(f"No s'ha pogut copiar '{p.name}' -> '{dest.name}': {e}")
            rows.append({
                'fitxer': p.name,
                'detected': det,
                'code': code,
                'elapsed_ms': res.summary.get('elapsed_ms'),
                'mode': res.summary.get('mode'),
            })
        except Exception as e:
            rows.append({'fitxer': p.name, 'detected': False, 'code': None, 'elapsed_ms': None, 'mode': mode, 'error': str(e)})
...
# --- Single file UI ---
def run_single_file_processing(uploaded_file, tipus_sel) -> None:
...
def render_single_file_ui() -> None:
    st.markdown("---")
...
def render_single_file_results() -> None:
    ...
    with st.expander("🔎 Resultats del Fitxer Individual", expanded=True):
        ...
        if detected:
            st.success(f"**Codi detectat:** {code}")
        else:
